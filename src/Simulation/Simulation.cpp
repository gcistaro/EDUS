...
Simulation::Simulation(std::shared_ptr<Simulation_parameters>& ctx__)
{
    ...
    Material material(ctx_->cfg().tb_file());
    int num_bands_ = material.H.get_Operator_R().get_nrows();

    //TODO: remove next line, it is redundant 
    Operator<std::complex<double>>::mpindex.initialize(ctx_->cfg().grid(), num_bands_ * num_bands_);

    auto propagator_parameters = PropagatorParametersFactory::create( ctx_->cfg() );

    //TODO: remove next line, static variable
    Operator<std::complex<double>>::SpaceOfPropagation = k;

    gridstructure_.initialize(ctx_->cfg().grid());
    decomposition_.initialize(ctx_->cfg().grid(), num_bands_, mpi::Communicator::world().size());
    kgradient_.initialize( propagator_parameters.gradient_space == R ? *gridstructure_.Rgrid() : *gridstructure_.kgrid() );
    electrons_.initialize(material, gridstructure_, decomposition_.mpindex(), kgradient_, 
                          propagator_parameters.gradient_space, 
                          electron::SystemParametersFactory::create( ctx_->cfg(), num_bands_ ));
    electron::print_pdos(electrons_.bandstructure(), "pdos.txt", decomposition_.kpool_comm());
    if ( ctx_->cfg().kpath().size() > 1 ) {
        /* NB: bands of the tight-binding model, the gap opened with opengap is not included */
        output::print("-> Printing band structure");
        electron::print_bandstructure_path(material.H, ctx_->cfg().kpath(), ".", mpi::Communicator::world().rank() == 0);
    }
    if ( std::abs(electrons_.parameters().opengap) > 0. ) {
        output::print("-> gap opened by ", Convert(electrons_.parameters().opengap, AuEnergy, ElectronVolt), " eV");
        output::print("It is recommended to restart the simulation with the file wannier_tb.dat that I will print");
        electrons_.print_wannier("wannier_tb.dat", gridstructure_, decomposition_.kpool_comm());
    }
    electrons_state_.initialize(gridstructure_, num_bands_, decomposition_.mpindex());

    //TODO: remove next lines
    Operator<std::complex<double>>::EigenVectors = &electrons_.bandstructure().U();
    Operator<std::complex<double>>::EigenVectors_dagger = &electrons_.bandstructure().Udagger();

    meanfield_.initialize( electron::MeanFieldParametersFactory::create( ctx_->cfg() ), 
                           gridstructure_, 
    ...
    propagator_.initialize( propagator_parameters, 
                            gridstructure_, 
                            electrons_, 
                            electrons_state_, 
                            meanfield_, 
    ...
    /* allocate device memory for the arrays we need on gpu and send them there */
    if( propagator_.processor() == device ) {
        electrons_.initialize_device();
        electrons_state_.initialize_device();
        propagator_.initialize_device();
        if( meanfield_.parameters().enabled ) {
            meanfield_.initialize_device();
        }
    }
}

void Simulation::convert_input_au()
{
    /* set times in a.u. */
...
//// ==         }
//// ==     }
//    const int n = H0.get_nblocks() * H0.get_nrows() * H0.get_ncols();
//    
//    /* Add coupling with laser */
//    /* H = H0 + E \cdot r*/
//    // == cblas_zcopy(n, H0.data(), 1, H.data(), 1);
//    // == cblas_zaxpy(n, las[0] + im*0., x.data(), 1, H.data(), 1);
//    // == cblas_zaxpy(n, las[1] + im*0., y.data(), 1, H.data(), 1);
//    // == cblas_zaxpy(n, las[2] + im*0., z.data(), 1, H.data(), 1);
//
//    /* Dynamical part ( electron-phonon ) */
//    /* H += g*delta_tau */
//    // == if( electron-phonon ) {
//    // ==     cblas_zgemv(
//    // ==         CblasRowMajor, CblasNoTrans,
//    // ==         n,
//    // ==         phonon_eigenvectors.total_size(),
//    // ==         1.+im*0.,
//    // ==         electronphonon.data(),
//    // ==         phonon_eigenvectors.total_size(),
//    // ==         phonon_eigenvectors.data(),
//    // ==         1,
//    // ==         1.+im*0.,
//    // ==         H.data(),
//    // ==         1);
...
void Simulation::Propagate()
{
    ...
    for (int it = 0; it < iFinalTime; ++it) {
        ...
        output_.write(propagator_.current_time());
        propagator_.step();
    }
...
//    Operator<std::complex<double>>::mpindex.initialize(MeshGrid::MasterRgrid.get_Size(), 
//                                                       material_.H.get_Operator_R().get_nrows() * material_.H.get_Operator_R().get_nrows());
...
//
//
//void Simulation::initialize_variables()
//{
//    ctx_->cfg().initialtime(Convert(ctx_->cfg().initialtime(), unit(ctx_->cfg().initialtime_units()),
//        AuTime));
//    ctx_->cfg().finaltime(Convert(ctx_->cfg().finaltime(), unit(ctx_->cfg().finaltime_units()),
...
//}
//
//void Simulation::initialize_lasers()
//{
//    output::print("-> initializing lasers");
...
//
//
//void Simulation::initialize_objects()
//{
//    // == systemmm       MeshGrid::MasterRgrid = MeshGrid(R, ctx_->cfg().grid());
//    // == systemmm   MeshGrid::MasterRgrid_GammaCentered = get_GammaCentered_grid(MeshGrid::MasterRgrid);
//    // == systemmm   bare_MasterRgridGammaCentered.initialize({MeshGrid::MasterRgrid.mpindex.get_nlocal(), 3});
//    // == systemmm   for( int iR_loc=0; iR_loc<MeshGrid::MasterRgrid.mpindex.get_nlocal(); iR_loc++ ) {
//    // == systemmm       int iR_glob = MeshGrid::MasterRgrid.mpindex.loc1D_to_glob1D(iR_loc);
//    // == systemmm       for (auto& ix : {0,1,2}) {
//    // == systemmm           bare_MasterRgridGammaCentered(iR_loc,ix) = MeshGrid::MasterRgrid_GammaCentered[iR_glob].get("Cartesian")[ix];
//    // == systemmm       }
//    // == systemmm   }
